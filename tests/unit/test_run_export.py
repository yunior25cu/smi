...
@pytest.fixture
def institutions():
    return [InstitutionConfig(id="SMI", nombre="SMI", plantilla=FIXTURE, hoja="DJ")]
...
def test_full_pipeline_writes_values_and_detects_all_discrepancies(tmp_path, institutions):
    ...
    # Mapa: 1->C2 y 2->D2 (validos), 99->F1 (huerfano: no esta en el catalogo).
    indicator_map = [
        IndicatorMapEntry(plasqlid=1, institucion_id="SMI", hoja="DJ", celda="C2"),
        IndicatorMapEntry(plasqlid=2, institucion_id="SMI", hoja="DJ", celda="D2"),
        IndicatorMapEntry(plasqlid=99, institucion_id="SMI", hoja="DJ", celda="F1"),
    ]
    ...
    dj = wb["DJ"]
    assert dj["C2"].value == 12345.67
    assert dj["D2"].value is None  # NULL: se escribe como celda vacia
    # F1 nunca se toca: 99 no esta en el catalogo, no hay valor para escribir.
...
def test_only_entries_for_the_target_institution_are_used(tmp_path, institutions):
    ...
    indicator_map = [
        IndicatorMapEntry(plasqlid=1, institucion_id="SMI", hoja="DJ", celda="C2"),
        IndicatorMapEntry(plasqlid=1, institucion_id="OTRA", hoja="DJ", celda="G99"),
    ]
